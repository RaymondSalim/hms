...
export async function generateInitialBillsForRollingBooking(booking: Pick<Booking, 'start_date' | 'fee' | 'second_resident_fee'> & {
    ...
    const isProrated = start_date.getDate() !== 1;

    // First bill (prorated or full)
    ...
    if (isProrated) {
        const secondBillStartDate = startOfMonth(addMonths(start_date, 1));
        const secondBillEndDate = endOfMonth(secondBillStartDate);
        const secondBillItems: BillItemUncheckedCreateWithoutBillInput[] = [{
        ...
        const secondBillAddonItems = processAddonsForPeriod(secondBillStartDate, secondBillEndDate, addOns ?? []);
        secondBillItems.push(...secondBillAddonItems);
